...
def _coordinate_sheet(elements: list) -> pd.DataFrame:
    rows = []
    skip = {"Building", "Site", "BuildingStorey"}

    for el in elements:
        if el["type"] in skip:
            continue
        rows.append({
            "Name": f"COORD-{el['name']}",
            "CreatedBy": CREATED_BY,
            "CreatedOn": NOW,
            "Category": "Point",
            "SheetName": "Component" if el["type"] != "Space" else "Space",
            "RowName": el["name"],
            "CoordinateXAxis": "TBC",
            "CoordinateYAxis": "TBC",
            "CoordinateZAxis": "TBC",
            "ClockwiseRotation": "TBC",
            "ElevationalRotation": "TBC",
            "YawRotation": "TBC",
            "ExternalSystem": "External source required",
            "ExternalObject": f"Ifc{el['type']}",
            "ExternalIdentifier": el["guid"],
        })
...
def _attribute_sheet(elements: list, enrichments: dict) -> pd.DataFrame:
    rows = []

    for el in elements:
        enrich = enrichments.get(el["guid"], {})

        for key, value in list(el.get("props", {}).items())[:20]:
            rows.append({
                "Name": key,
                "CreatedBy": CREATED_BY,
                "CreatedOn": NOW,
                "Category": "IFC Property",
                "SheetName": "Component" if el["type"] not in {"Space", "BuildingStorey"} else el["type"],
                "RowName": el["name"],
                "Value": value,
                "Unit": "TBC",
                "AllowedValues": "TBC",
                "Description": f"IFC property extracted from {el['type']}",
                "ExternalSystem": "IFCOpenShell",
                "ExternalObject": f"Ifc{el['type']}",
                "ExternalIdentifier": el["guid"],
            })

        for key in ["lifeYears", "replacementCost", "maintenance", "frequency"]:
            rows.append({
                "Name": key,
                "CreatedBy": CREATED_BY,
                "CreatedOn": NOW,
                "Category": "AI or external asset data",
                "SheetName": "Component",
                "RowName": el["name"],
                "Value": enrich.get(key, "TBC"),
                "Unit": "Years" if key == "lifeYears" else "TBC",
                "AllowedValues": "TBC",
                "Description": f"{key} attribute for {el['type']}",
                "ExternalSystem": "OpenAI GPT / External source",
                "ExternalObject": f"Ifc{el['type']}",
                "ExternalIdentifier": el["guid"],
            })
...
def _connection_sheet(elements: list) -> pd.DataFrame:
    rows = []
    components = [e for e in elements if e["type"] not in {"Building", "Site", "BuildingStorey", "Space"}]
    ...
    for i in range(len(components) - 1):
        ...
        rows.append({
            ...
            "RowName2": b["name"],
            "ConnectionType": "TBC",
            "RealizingElement": "TBC",
            "PortName1": "TBC",
            "PortName2": "TBC",
            "Description": "External source required to confirm physical or logical connection",
            "ExternalSystem": "External source required",
            "ExternalObject": "IfcRelConnects / system relationship",
            "ExternalIdentifier": "TBC",
        })

    return pd.DataFrame(rows)


def _issue_sheet(elements: list) -> pd.DataFrame:
    rows = []

    for el in elements:
        if el["type"] in {"Building", "Site", "BuildingStorey"}:
            continue
        rows.append({
            "Name": f"ISSUE-{el['name']}",
            "CreatedBy": CREATED_BY,
            "CreatedOn": NOW,
            "Category": "Data quality",
            "SheetName": "Component" if el["type"] != "Space" else "Space",
            "RowName": el["name"],
            "Risk": "External source required for full verification",
            "Chance": "Medium",
            "Impact": "Medium",
            "Owner": "Information Manager",
            "Mitigation": "Review against external asset database, O&M manuals, survey data, and COBie requirements",
            "ExternalSystem": "Validation engine",
            "ExternalObject": f"Ifc{el['type']}",
            "ExternalIdentifier": el["guid"],
        })
...
def build_cobie(elements: list, enrichments: dict) -> dict[str, pd.DataFrame]:
    ...
    sheets = {
        ...
        "Coordinate": _coordinate_sheet(elements),
        "Attribute":  _attribute_sheet(elements, enrichments),
        "Connection": _connection_sheet(elements),
        "Issue":      _issue_sheet(elements),
    }
    # Drop empty sheets
