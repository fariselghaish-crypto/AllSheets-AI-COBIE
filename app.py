...
from pipeline.external_sources import load_external_database, merge_external_sources, external_source_template
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### 🔗 External Source Database")
    st.caption(
        "Optional Excel database to fill missing/TBC COBie fields from O&M manuals, "
        "asset registers, CMMS, surveys, product databases, or document control systems."
    )

    external_db_file = st.file_uploader(
        "Upload external database Excel",
        type=["xlsx"],
        help="Optional workbook with COBie sheet names and matching columns.",
        key="external_db_upload"
    )

    st.download_button(
        label="⬇ Download External Database Template",
        data=external_source_template(),
        file_name="external_cobie_database_template.xlsx",
...
if run and uploaded and api_key:
    ...
    with st.status("⬡ Stage 3 — Building COBie sheets…", expanded=True) as status:
        try:
            ...
            external_sheets = load_external_database(external_db_file)
            cobie, external_logs = merge_external_sources(cobie, external_sheets)
    ...
    tab_cobie, tab_val, tab_ext, tab_log = st.tabs([
        "📋 COBie Sheets",
        "✅ Validation Report",
        "🔗 External Sources",
        "📜 Log",
    ...
    with tab_cobie:
        ...
        sheet_order = [
            ...
            "Type", "Component", "System", "Job", "Resource", "Spare",
            "Contact", "Document", "Coordinate", "Attribute", "Connection", "Issue"
        ]
    ...
    with tab_ext:
        st.markdown("### External Source Retrieval Log")

        if "external_logs" in locals():
            for line in external_logs:
                st.text(line)
        else:
            st.info("No external source processing was completed.")

        st.markdown("---")
        st.markdown("### Cells Requiring External Sources")
        st.info(
            "In the exported Excel workbook, cells marked as TBC or 'External source required' "
            "should be completed using external sources such as O&M manuals, asset registers, "
            "CMMS data, survey data, product databases, maintenance schedules, commissioning "
            "records, and document control systems."
        )
